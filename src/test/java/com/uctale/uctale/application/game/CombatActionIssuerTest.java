...
class CombatActionIssuerTest {
    ...
    @Test
    @DisplayName("server-issued combat actions는 active player turn에서만 발급된다")
    void issue_OnlyForActivePlayerTurn() {
        ...
        assertThat(issuer.issue(active)).extracting(a -> a.type()).containsExactly(ActionType.COMBAT_PASS, ActionType.COMBAT_ESCAPE);
        assertThat(issuer.issue(active)).allMatch(a -> a.arguments().get("encounterId").equals("enc-1"));
    }
}
