...
class GameStateCodecCompatibilityTest {
    ...
    @Test
    @DisplayName("현재 schema v8에서 stats가 누락된 손상 snapshot은 기본값으로 숨기지 않는다")
    ...
    @Test
    @DisplayName("현재 schema v8에서 inventory가 누락된 손상 snapshot은 빈 inventory로 숨기지 않는다")
    void currentSchemaMissingInventory_FailsExplicitly() {
        String player = player();
        String json = "{\"schemaVersion\":8,\"rulesetVersion\":1,\"state\":{\"turnNumber\":1," + player
                + ",\"worldState\":{\"premise\":\"세계관\",\"flags\":{}},\"storyMemory\":{\"canonicalFacts\":[],\"rollingSummary\":\"\",\"recentTurns\":[]},\"combatEncounter\":null,\"abilityState\":{\"cooldowns\":{}},\"questState\":" + questState() + "}}";
    ...
    @Test
    @DisplayName("현재 schema v8에서 vitals가 누락된 손상 snapshot은 기본 vitals로 숨기지 않는다")
    ...
    @Test
    @DisplayName("현재 schema v8에서 abilityState가 누락되면 empty cooldown으로 숨기지 않는다")
    void currentSchemaMissingAbilityState_FailsExplicitly() {
        String json = baseWithoutTail(player()) + ",\"questState\":" + questState() + "}}";
        assertThatThrownBy(() -> codec.deserialize(json))
                .isInstanceOf(GameStateSnapshotException.class).hasMessageContaining("abilityState/cooldowns");
    }

    @Test
    @DisplayName("현재 schema v8에서 questState가 누락되면 empty quest로 숨기지 않는다")
    void currentSchemaMissingQuestState_FailsExplicitly() {
        String json = baseWithoutTail(player()) + ",\"abilityState\":{\"cooldowns\":{}}}}";
        assertThatThrownBy(() -> codec.deserialize(json))
    ...
    private String baseWithoutTail(String playerField) {
        return "{\"schemaVersion\":8,\"rulesetVersion\":1,\"state\":{\"turnNumber\":1," + playerField
