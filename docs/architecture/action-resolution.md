...
현재 action type은 `NARRATIVE_CHOICE`, `SKILL_CHECK`, `COMBAT_ATTACK`, `COMBAT_ABILITY`, `COMBAT_PASS`, `COMBAT_ESCAPE`다. 범용 Rule Engine을 만들지 않고 inventory/equipment, HP/MP/status, combat/ability, quest/objective/flag를 작은 타입 안전 규칙으로 유지한다.
...
application canonical 진입점이다. `ActionResolver`의 기본/Skill Check/Attack/inventory/effect resolution을 받은 뒤 `QuestRules.apply`를 정확히 한 번 적용한다.

Quest 규칙은 provider prose를 읽지 않고 이미 확정된 action/state changes만 관찰한다.
...
따라서 quest progress와 flag는 Narrative provider 호출 전에 `GameResult`와 next `GameState`에 포함된다. 자세한 규칙은 [quest-objective-flags.md](./quest-objective-flags.md)를 기준으로 한다.
...
`GameService`는 개별 inventory/vitals/combat/ability/quest 공식에 의존하지 않는다.
...
Ability audit은 `game_log.combat_changes_json`, quest/objective/flag audit은 `game_log.quest_changes_json`에 저장한다. `GameStateRecovery`는 snapshot이 없을 때 모든 typed ledger audit을 순서대로 replay한다.

## Narrative provider 전후 경계

Provider 호출 전에 resolved action, 판정 결과, inventory/vitals/combat/ability 변화, quest/objective/flag 변화, canonical next state가 확정된다. `NarrativeContext`는 이 상태를 read-only projection하며 `PlayerAction.token`은 제외한다.

provider prose는 HP/MP/status/target/cooldown, quest 상태/objective progress, World/Event Flag를 만들거나 변경하는 canonical 근거가 아니다. Story transcript도 이미 확정된 rule state를 변경하지 않는다.
...
- snapshot schema v8은 `abilityState`와 `questState`를 명시적으로 저장한다.
- v6→v7은 empty ability state, v7→v8은 empty quest/flag state만 deterministic하게 추가한다.
- 기존 `WorldState.flags`는 legacy 의미를 추측해 새 typed `WorldFlag`로 승격하지 않는다.
- legacy `game_log.quest_changes_json = NULL`은 quest/flag 변화가 없었던 turn으로 읽는다.
